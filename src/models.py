...
class Network(nn.Module):
    def __init__(self, state_dim, action_dim, num_fc_1, num_fc_2, num_fc_last,
                 with_bn: bool = True, is_actor: bool = True):
        super(Network, self).__init__()
        self.is_actor = is_actor
        self.with_bn = with_bn

        self.fc1 = nn.Linear(state_dim * 2, num_fc_1)
        self.bn = nn.BatchNorm1d(num_fc_1) if self.with_bn else None
        if self.is_actor:
...
class Actor(Network):
    def __init__(self, state_dim, action_dim, num_fc_1, num_fc_2, with_bn: bool):
        super(Actor, self).__init__(state_dim, action_dim, num_fc_1, num_fc_2, action_dim,
                                    with_bn=with_bn, is_actor=True)


class Critic(Network):
    def __init__(self, state_dim, action_dim, num_fc_1, num_fc_2, with_bn: bool):
        super(Critic, self).__init__(state_dim, action_dim, num_fc_1, num_fc_2, 1,
                                     with_bn=with_bn, is_actor=False)
